...
- Stack baseline: macOS/Xcode SplitFlap screensaver, with bootstrap-managed governance files.
- CI baseline: fast PR checks stay cheap and shell-safe; macOS/Xcode validation runs on the macOS runner; release verification must compile SplitFlap with `xcodebuild`.
- Self-hosted runner policy: shell-safe jobs may use `[self-hosted, synology, shell-only, public]`; Xcode jobs must use the macOS ARM64 Xcode runner.
- Add or update tests/checks for every interactive, branching, or operator-facing behavior change.
...
- Treat `SplitFlap/`, `SplitFlap.xcodeproj/`, `Makefile`, and the Xcode build command as first-class app validation surfaces, not generic placeholder content.
